...
Complété par: Jonathan Mehmannavaz et Johny Huynh de l'équipe 3 sur visual studio.
...
int solution_systeme_2eq(double a1, double b1, double c1, double a2, double b2, double c2, double* x, double* y)
{
    ...
    ///*** à compléter ***///
    d = a1 * b2 - b1 * a2;//on calcule le determinant de la matrice formé par les deux équation.

    if (d == 0)//le determinant de la matrice donne 0 l'équation n'admet pas de solution, x et y = (0,0)
    {
        *x = 0;
        *y = 0;
        result = 0;
    }
    else//le système d'équation admet une solution
    {
        dx = c1 * b2 - b1 * c2;
        dy = a1 * c2 - c1 * a2;
        *x = dx / d;
    ...
    double* a, double* b, double* c)
{
    ...
    ///*** à compléter ***///
    if (x2 == x1)//Si les point forment une droite vertitcal
    {
        *a = 1;
        *b = -x2;
        *c = 0;
    ...
    else if (y1 == y2)//Si les deux point forment une droite horizontale
    {
        *a = 0;
        *b = y2;
        *c = 1;
    ...
    else if (y1 != y2)//Si les point forme une pente, on peut trouver l'équation
    {
    ...
    return result;// renvoie 1 si on peut trouver une equation
}
// ---------------------------------------------
    ...
    double* x, double* y)
{
    ...
    double d, dx, dy;             
    d = -a1 * c2 - c1 * -a2;//on calcule le determinant de la matrice

    if (d == 0)                   //les droite n'adment pas de solution
    {
        *x = 0;

        *y = 0;
    }
    else                          //les droites admettent une solution
    {
        dx = b1 * c2 - c1 * b2;
...
/* fonction: intersection_segments
    Description:    intersection de segments données par les coordonnées de leurs
                    ...
                    segment 2   entre A2=(ax2,ay2) et B=(bx2,by2)
...
{
    int result = 0;
    double a1, b1, c1;              // L'équation de la droite du segment 1
    double a2, b2, c2;              // L'équation de la droite du segment 2
    double point_x, point_y;      //Point d'intersection des deux segments

    if (equation_droite(ax1, ay1, bx1, by1, &a1, &b1, &c1) == 1 && equation_droite(ax2, ay2, bx2, by2, &a2, &b2, &c2) == 1)     //calcule des coefficients des segments
    {
        if (intersection_droites(a1, b1, c1, a2, b2, c2, &point_x, &point_y) == 1)    //calcul du point d'intersection des deux segments
        {
            if (element_du_segment(ax1, ay1, bx1, by1, point_x, point_y) == 1 && element_du_segment(ax2, ay2, bx2, by2, point_x, point_y) == 1)    //e point trouver à l'étape précédente fait partie des deux segments
            {
                *x = point_x;
                *y = point_y;
                result = 1;  
...
{
    int s = 1;
    if (x < 0)// renvoie -1 si x est negatif
    {
        s = -1;
    }
    return s; //si x est >=0 , la focntion renvoie tout de suite 1.
}
